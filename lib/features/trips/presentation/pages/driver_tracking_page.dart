...
class _DriverTrackingPageState extends State<DriverTrackingPage>
  ...
  Future<void> _openUpdateStatusSheet(Trip trip) async {
    ...
    await showModalBottomSheet<bool>(
      ...
      // Constrained sheets are centred by the framework, so on an iPad this
      // rises as a panel rather than a 1300pt-wide slab.
  ...
  Widget _buildBody() {
    ...
    // Status changes belong to a running trip - completed, failed and paused
    // trips show a banner in the scroll content instead of the button.
    final canChangeStatus = !trip.isShippingDone && !isFailed && !isPaused;

    // No pull-to-refresh here: every drag on the map just moves the map.
    // Refreshing lives in the app bar instead, and the screen re-reads itself
    // on a timer anyway.
    return Column(
      children: [
        Expanded(
            ...
            child: Column(
              ...
              children: [
                SizedBox(
                  ...
                  // The map is the one element that earns the extra room an
                  // iPad gives, so it keeps the full width and gets more
                  // height instead of leaving the route squeezed into a
                  // phone-sized strip at the top of a much taller screen.
                  height: context.isWideLayout ? 460 : 380,
                  child: AnimatedBuilder(
                    ...
                    builder: (context, _) {
                      ...
                      return LiveTrackingMap(
                        ...
                        // A downward drag should pan the map, not expand it.
                        expandOnDragDown: false,
                      );
                    },
                  ),
                ),
                        ...
                        AnimatedBuilder(
                          ...
                          builder: (context, _) => _LocationAccessBanner(
                            ...
                            isTrackingNotificationHidden:
                                (_locationController?.isLiveTracking ??
                                    false) &&
                                !(_locationController
        ...
        // Pinned to the bottom of the screen so it's always in reach, however
